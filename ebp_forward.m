...
load('gtcs_math_sine_400.mat'); % load training data
...
NUM_EPOCHS = 25; ETA = .01; % set training parameters
sz_input = size(input_training); sz_output = size(output_training); % get number of training pairs
NUM_INP = sz_input(2); NUM_HID = 10; NUM_OUT = sz_output(2); % set input, hidden and output layer sizes
BIAS = -.1; % set bias
error_epochs = zeros(NUM_EPOCHS,1); % initialize epoch error vector
Phi = zeros(NUM_HID+NUM_OUT, max(NUM_INP,NUM_HID+1)); % initialize weight matrix
...
Phi
plot(error_epochs,'DisplayName','error_epochs_forward_ep_25__eta_d01__hid_10'); grid minor; legend;